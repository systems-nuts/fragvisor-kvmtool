...
int pci_shmem__get_local_irqfd(struct kvm *kvm)
{
	...
	if (local_fd == 0) {
		...
		printf("<%d> %s(): eventfd() fd fd %d (?/SYSC_eventfd2)\n\n",
...
int pci_shmem__init(struct kvm *kvm)
{
    ...
    //if (pop_get_nid())
	//	return 0;
	POP_DPRINTF(pop_get_nid(), "\t\t[<%d>/%d] %s(): JACK JACK\n\n",
    ...
    /* */
    char *mem2;
    u64 phys_addr2 = 0x10000000; //(0xc8000000) /* 3G */
    u64 size2 = 16 << MB_SHIFT;
    printf("Proj2:\n");
    printf("phys_addr: %llu\n", phys_addr2);
    printf("size: %llu\n", size2);
    //if(size2 < 0) {
    if (0) {
        mem2 = setup_shmem("/kvm_shmem", size2, 1);
        if (mem2 == NULL)
            return -EINVAL;
        kvm__register_mem(kvm, phys_addr2, size2, mem2); /* expose vm mem */
    }
#endif
