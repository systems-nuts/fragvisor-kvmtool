...
print('XXX.replace(\'\\n\',\'\') works only for py2 not py3 => so use XXX.decode(\'utf-8\').replace(\'\\n\',\'\')')
...
for node in range(0, 3): # server 0 ~ 4 # e.g. range(0,4) = [0,1,2,3]
    for op in ['R', 'W']:
        ...
        with open(TRACE) as f:
            for l in f.readlines(): # py3
            #for l in f.xreadlines(): # py2
                e = l.strip().split()
                if len(e) < 2: continue;
                if e[0][0] == "#": continue;
                e = l.split()
...
with open(TRACE) as f:
    ...
    #for l in f.xreadlines(): # py2
        ...
        if len(e) < 2: continue;
        if e[0][0] == "#": continue;
        # kvm-vcpu-0-32390 [000] ....   866.057005: kvm_page_fault_ext: rip ffffffff8165147c address 1e04500 vaddress ffffffff81e04500 error_code dab (rw- R-X- pk roEx)
        del e[:3]
        if t_ofs == 0: t_ofs = float(e[0][:-1]);
        # we are interested in:
        if e[1][:-1] == "kvm_page_fault_ext":
            rip_txt = e[2] # don't care
            rip = e[3]
            addr_txt = e[4] # don't care
            addr = e[5]
            vaddr_txt = e[6] # don't care
            vaddr = e[7]
            err_txt = e[8] # don't care
            err = e[9]
            reason1 = e[10]
            reason2 = e[11]
            reason3 = e[12]
            ...
            dict_vmdsm.setdefault('%d %d %d %d - %s %s %s %s  %s %s %s %s' % (int(rip, 16), int(addr, 16), int(vaddr, 16), int(err, 16), rip, addr, vaddr, err, reason1, reason2, reason3, reason4), 0)
            dict_vmdsm['%d %d %d %d - %s %s %s %s  %s %s %s %s' % (int(rip, 16), int(addr, 16), int(vaddr, 16), int(err, 16), rip, addr, vaddr, err, reason1, reason2, reason3, reason4)] += 1
            #'%d %d %c %s %s' % (int(pgfault[:-3], 16), int(rip[:-3], 16), op, pgfault, rip)] += 1
    # end for
    print('Writing to %s...' % VMDSM_CENSUS_FILE)
    ff = open('%s' % VMDSM_CENSUS_FILE, 'w')
...
# next process
print('invoke 1-ins_ptr_process.py...')
os.system('./1-ins_ptr_process.py %s %s' % (INS_CENSUS_FILE, APP_SOURCE_CODE))
print('invoke 2-pgfault_process.py...')
os.system('./2-pgfault_process.py %s %s' % (PGFAULT_CENSUS_FILE, APP_SOURCE_CODE))
#print('invoke prefetch_process.py...')
#os.system('./prefetch_process.py %s %s' % (PREFETCH_CENSUS_FILE, APP_SOURCE_CODE))
os.system('./4-vmdsm_process.py %s %s' % (VMDSM_CENSUS_FILE, APP_SOURCE_CODE))

# auto re-get the results
ff = open('%s_result.sh' % TRACE, 'w')
ff.write('#! /bin/bash\n')
ff.write('./1-ins_ptr_process.py %s %s\n' % (INS_CENSUS_FILE, APP_SOURCE_CODE))
ff.write('./2-pgfault_process.py %s %s\n' % (PGFAULT_CENSUS_FILE, APP_SOURCE_CODE))
ff.write('./4-vmdsm_process.py %s %s\n' % (VMDSM_CENSUS_FILE, APP_SOURCE_CODE))

#USER=`whoami`
#USER = os.system('whoami')
USER = subprocess.check_output(['whoami'])
#USER_TRIM = USER.replace('\n','') #py2
USER_TRIM = USER.decode('utf-8').replace('\n','') #py3
