...
int main(int argc, char *argv[])
{
  ...
  if (argc == 2) {
//	  POPCORN_CONFIG_NODES = atoi(argv[1]);

    // user define single
	/* set affinity */
	cpu_set_t cpuset;
	CPU_ZERO(&cpuset);
	CPU_SET(atoi(argv[1]), &cpuset); /* Start from 0 */
	sched_setaffinity(0, sizeof(cpuset), &cpuset);
    printf("./%s goin to run on <%d>\n", argv[0], atoi(argv[1]));

  } else {
	printf("./%s <pin on cpu> (start from 0)\n");
	return -1;
  }
//  if (argc >= 3) {
//	  POPCORN_CONFIG_CORES_PER_NODE = atoi(argv[2]);
//  }
//  omp_set_num_threads(POPCORN_CONFIG_NODES * POPCORN_CONFIG_CORES_PER_NODE);
  ...
  //POPCORN_OMP_MIGRATE_START(1);
  #pragma omp parallel default(shared) private(k,kk,t1,t2,t3,t4,i,ik,x1,x2,l)
  {
  	//printf("[%d] on omp_get_popdorn_tid %d\n", popcorn_gettid(), omp_get_popdorn_tid());
  	//printf("[%d] omp t# [%d]\n", popcorn_gettid(), omp_get_thread_num());
...
//    sched_setaffinity(0, sizeof(cpuset), &cpuset);
	...
	//popcorn_setaffinity(omp_get_thread_num());


//  	printf("(%d/%d)\n", omp_get_thread_num(), omp_get_num_threads()); /* multiple but all on <*> */
	// set_affinity();
    for (i = 0; i < NQ; i++) {
      qq[i] = 0.0;
    }

    #pragma omp for reduction(+:sx,sy) nowait schedule(static)
    for (k = 1; k <= np; k++) {
//      if (first[omp_get_thread_num()]) {
//	    printf("(%d/%d) - k %d / %d\n",
//	  		omp_get_thread_num(), omp_get_num_threads(), k, np);
//		first[omp_get_thread_num()] = false;
//	  }
      kk = k_offset + k; 
      t1 = S;
      t2 = an;
