...
static void *disk_image__thread(void *param)
{
    ...
    POP_PRINTF("\t[%d/%d] %s(): this thread \"disk-image-io\"\n",
				   ...
				   int use_mmap)
{
	...
	*disk = (struct disk_image) {
...
#ifdef CONFIG_POPCORN_HYPE
		POP_PRINTF("\t\t<%d> [%d] %s(): disk->priv %p\n\n",
				pop_get_nid(), popcorn_gettid(), __func__, disk->priv);
#endif
	}
...
#ifdef CONFIG_HAS_AIO
	{
		...
		if (disk->evt < 0)
			return ERR_PTR(disk->evt);

#ifdef CONFIG_POPCORN_HYPE
		POP_PRINTF("\t\t<%d> [%d] %s(): eventfd() disk->evt fd %d (14/SYSC_eventfd2)\n\n",
									pop_get_nid(), popcorn_gettid(), __func__, disk->evt);
		dump_stack();
		// <*> broadcast
		...
		for (i = 1; i < nodes; i++) {
			...
			POP_DPRINTF(pop_get_nid(), "\t\t<*> -> <%d> [%d] %s(): "
						"eventfd() disk->evt fd %d (14/SYSC_eventfd2)\n\n",
						pop_get_nid(), popcorn_gettid(), __func__, disk->evt);
			if (disk->evt < 0)
				return ERR_PTR(disk->evt);
			migrate(0, NULL, NULL);
...
#ifdef CONFIG_POPCORN_HYPE
		POP_PRINTF("\t\t<*><%d> [%d] %s(): disk_image__thread() %p\n\n",
			pop_get_nid(), popcorn_gettid(), __func__, &disk_image__thread);
#endif
...
static struct disk_image *disk_image__open(const char *filename, bool readonly, bool direct)
{
...
#ifdef CONFIG_POPCORN_HYPE
	// <*>
	POP_PRINTF("\t<%d> [%d] %s(): open(\"%s\") fd %d (13)\n",
			pop_get_nid(), popcorn_gettid(), __func__, filename, fd);

	int i, nid = pop_get_nid();
	BUG_ON(nid);
	for (i = 1; i < nodes; i++) {
		if (i == nid) { continue; } // redundant for node 0 but generalized
		migrate(i, NULL, NULL);
		fd = open(filename, flags);
		POP_PRINTF("\t\t<*> -> <%d> [%d] %s(): open(\"%s\") fd %d (13)\n",
					pop_get_nid(), popcorn_gettid(), __func__, filename, fd);
	...
	/* raw image ?*/
	disk = raw_image__probe(fd, &st, readonly); // => disk/raw.c => disk_image__new
	if (!IS_ERR_OR_NULL(disk))
		return disk;

#ifdef CONFIG_POPCORN_HYPE
	// <*>
	POP_PRINTF("\t<%d> [%d] %s(): close(\"%s\") fd %d (13)\n",
			pop_get_nid(), popcorn_gettid(), __func__, filename, fd);
#endif
	...
	for (i = 1; i < nodes; i++) {
		...
		if (close(fd) < 0)
			pr_warning("close() failed");
		POP_DPRINTF(pop_get_nid(), "\t\t<*> -> <%d> [%d] %s(): close(\"%s\") fd %d (13)\n",
								pop_get_nid(), popcorn_gettid(), __func__, filename, fd);
		if (fd < 0)
			return ERR_PTR(fd);
		migrate(0, NULL, NULL);
...
#ifdef CONFIG_POPCORN_HYPE
	POP_PRINTF("\t\t<%d> %s: Jack test disk->fd %d\n",
				pop_get_nid(), __func__, disk->fd);
#endif
//#else
//	int i, nid = pop_get_nid();
//	BUG_ON(nid);
...
//		migrate(i, NULL, NULL);
//		fd = close(disk->fd);
//		POP_PRINTF("\t\t<*> -> <%d> %s(): open(\"%s\") fd %d (13)\n",
//							pop_get_nid(), __func__, filename, fd);
//		if (fd < 0)
//			return ERR_PTR(fd);
//		migrate(0, NULL, NULL);
//	}
