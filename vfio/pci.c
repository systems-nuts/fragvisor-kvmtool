				     ...
				     struct vfio_pci_msi_entry *entry)
{
	...
	if (entry->eventfd < 0) {
	   ...
	   POP_PRINTF("<%d> %s(): eventfd() entry->eventfd fd %d (?/SYSC_eventfd2)\n\n",
									   pop_get_nid(), __func__, entry->eventfd);
#endif
...
#ifdef CONFIG_POPCORN_HYPE
	POP_PRINTF("<%d> %s(): eventfd() trigger_fd fd %d (?/SYSC_eventfd2)\n\n",
								   pop_get_nid(), __func__, trigger_fd);
#endif
	if (trigger_fd < 0) {
...
#ifdef CONFIG_POPCORN_HYPE
	POP_PRINTF("<%d> %s(): eventfd() unmask_fd fd %d (?/SYSC_eventfd2)\n\n",
								   pop_get_nid(), __func__, unmask_fd);
#endif
	if (unmask_fd < 0) {
