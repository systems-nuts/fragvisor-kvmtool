...
static int vfio_configure_device(struct kvm *kvm, struct vfio_device *vdev)
{
	...
	POP_PRINTF("\t\t[[[testing]]] vdev->fd %d handle me?\n", vdev->fd);
...
static int vfio_container_init(struct kvm *kvm)
{
...
#else
	int __vfio_container = INT_MIN;
	if (!pop_get_nid())
		vfio_container = open(VFIO_DEV_NODE, O_RDWR);
	else 
		__vfio_container = open(VFIO_DEV_NODE, O_RDWR);
	if (pop_get_nid()) {
		if (__vfio_container != vfio_container) {
				//POP_SANI_PRINTF(pop_get_nid(),
				//	"(BUG) vfio_container not matching %d %d\n",
				//						__vm_fd, kvm->vm_fd);
				BUG_ON(1);
		}
	}
	POP_FS_PRINTF(pop_get_nid(), "\t\t<%d> %s(): vfio_container %d\n",
							pop_get_nid(), __func__, vfio_container);
#endif
	if (vfio_container == -1) {
		ret = errno;
	...
	for (i = 0; i < kvm->cfg.num_vfio_devices; ++i) {
		...
		ret = vfio_device_init(kvm, &vfio_devices[i]); /* sync inside */
		if (ret)
			return ret;
	}
#ifdef CONFIG_POPCORN_HYPE
skip:
	pthread_barrier_wait(&barrier_vfio_heap);
	POP_DBG_PRINTF(pop_get_nid(),
			"\t\t<%d> %s(): TODO inside vfio_device_init()",
										pop_get_nid(), __func__);
#endif
	...
	if (ioctl(vfio_container, VFIO_SET_IOMMU, iommu_type)) {
		...
		pr_err("Failed to set IOMMU type %d for VFIO container",
		       iommu_type);
#else
		POP_ERR_PRINTF(pop_get_nid(),
			"\t\t<%d> %s(): Failed to set IOMMU type %d for VFIO container",
										pop_get_nid(), __func__, iommu_type);
#endif
		return ret;
	} else {
...
static int vfio__init(struct kvm *kvm)
{
...
#ifdef CONFIG_POPCORN_HYPE
	if (!pop_get_nid()) {
		POP_PRINTF("\t[<%d>/%d] %s(): DOING NOTHING %u\n",
				pop_get_nid(), popcorn_gettid(), __func__, kvm->cfg.num_vfio_devices);
	} else {
		POP_PRINTF("\t[<%d>/%d] %s(): TODO !!!! (NEVER SEEN)\n",
	...
	/* WILL NEVER REACH */
	pr_err("BUG BUG BUG shouldn't reach here");
	BUG_ON(1);
	POP_SANI_PRINTF(pop_get_nid(), "\t[<%d>/%d] %s(): TODO HANDLE ME\n",
								pop_get_nid(), popcorn_gettid(), __func__);
#ifndef CONFIG_POPCORN_HYPE
	...
	if (!pop_get_nid())
		vfio_devices = calloc(kvm->cfg.num_vfio_devices, sizeof(*vfio_devices));
	pthread_barrier_wait(&barrier_vfio_malloc);
#endif
	if (!vfio_devices)
		return -ENOMEM;
	...
	if (!pop_get_nid())
		...
		__ret = vfio_container_init(kvm); /* sync */

	pthread_barrier_wait(&barrier_container_init);
	if (pop_get_nid()) {
		if (__ret != ret) {
			POP_SANI_PRINTF(pop_get_nid(),
				"(BUG) ret not matching %d %d\n", __ret, ret);
			BUG_ON(1);
		}
	}
	POP_DBG_PRINTF(pop_get_nid(), "\t<%d> %s(): ret %d\n",
								pop_get_nid(), __func__, ret);
#endif
	if (ret)
		return ret;
	...
	if (ret)
		return ret;
#ifdef CONFIG_POPCORN_HYPE
	pthread_barrier_wait(&barrier_reserve_region_done);
#endif
