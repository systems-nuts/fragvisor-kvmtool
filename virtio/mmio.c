				      ...
				      struct virtio_device *vdev, u32 vq)
{
    ...
    POP_PRINTF("<%d> %s(): eventfd() ioevent.fd fd %d vq %u (?/SYSC_eventfd2)\n\n",
									pop_get_nid(), __func__, ioevent.fd, vq);
#endif
#ifdef CONFIG_POPCORN_HYPE
	POP_PRINTF("\t\t[BUG net %d/%d] %s: %s(): "
			"eventfd() ioevent.fd fd %d vq %u (?/SYSC_eventfd2)\n\n\n",
			pop_get_nid(), popcorn_gettid(), __FILE__, __func__, ioevent.fd, vq);
#endif
...
#ifdef CONFIG_POPCORN_HYPE
	POP_PRINTF("\t\t[BUG net %d/%d] %s: %s(): mmio should not happen\n",
...
#ifdef CONFIG_POPCORN_HYPE
    POP_PRINTF("\t[%d/%d] %s(): going to call virtio_mmio_config_%s()\n",
		pop_get_nid(), popcorn_gettid(), __func__, is_write ? "out" : "in");
#endif
...
#ifdef CONFIG_POPCORN_HYPE
    POP_PRINTF("[%d] %s: %s(): JACK device__register() { rb_tree inster }\n\n",
											pop_get_nid(), __FILE__, __func__);
#endif
