				   ...
				   struct kvm_irq_routing_entry *entry)
{
	...
	if (nid) {
		printf("Warning: at %s line %d\n", __FILE__, __LINE__);
		printf("Warning: at %s line %d\n", __FILE__, __LINE__);
		printf("Warning: [ft] remote should not be here "
				"at %s line %d\n", __FILE__, __LINE__);
		printf("Warning: at %s line %d\n", __FILE__, __LINE__);
		printf("Warning: at %s line %d\n", __FILE__, __LINE__);
		//dump_stack();
		...
		for (i = 1; i < nodes; i++) {
			if (i == nid) { continue; }
			POP_PRINTF("\t\t20 22[*] net: %s: %s(): asking to replay "
					"ioctl(KVM_SET_GSI_ROUTING) at [%d]\n",
					__FILE__, __func__, i);
			migrate(i, NULL, NULL); /**********************************/

			//POP_DPRINTF(pop_get_nid(), "\t\t20 22 [from *] => <%d> [%d] %s(): "
			//            "goin replay ioctl(KVM_SET_GSI_ROUTING)\n\n",
			//            pop_get_nid(), popcorn_gettid(), __func__);
			POP_PRINTF("\t\t20 22 [from *] => <%d> [%d] %s(): "
						"replaying ioctl(KVM_SET_GSI_ROUTING)\n",
						pop_get_nid(), popcorn_gettid(), __func__);
			ioctl(kvm->vm_fd, KVM_SET_GSI_ROUTING, irq_routing);

			migrate(0, NULL, NULL); /**********************************/
			POP_PRINTF("\t\t[*] net: ioctl(KVM_SET_GSI_ROUTING) done at remote %d\n", i);
		}
	}
...
#ifdef CONFIG_POPCORN_HYPE
	POP_PRINTF("\t\t[net %d/%d/%d] %s: %s(): msi (WATCHOUT HEAP DATA) "
			"->update_route()\n",
...
#ifdef CONFIG_POPCORN_HYPE
    POP_PRINTF("\t\t[net %d/%d/%d] %s: %s(): msi -> update_route()\n",
	            pop_get_nid(), getpid(), popcorn_gettid(), __FILE__, __func__);
#endif
...
#ifdef CONFIG_POPCORN_HYPE
	POP_PRINTF("\t\t\t[net %d/%d] VIRTIO_MSI_QUEUE_VECTOR(22)->%s: %s():\n",
