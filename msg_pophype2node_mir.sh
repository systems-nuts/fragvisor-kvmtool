...
echo "ATTENTION: run before the remote node is up, will not update remote config.h"
...
#NODES="echo5 echo4 echo0" # 5 4 0 1
NODES="mir7 mir6" # 5 4
...
do
	...
	ssh $node "sync"
	ssh $node "cp $scripts_location/config_pophype2node_mir_76.h $kernel_location/msg_layer/config.h"
	echo "sync $node"; echo;
done
...
do
	...
	while [[ $ret != 0 ]];
	do
		echo "====== (copy this part to msg_34(mir/echo))"
		echo " [$node]: test target IB working or not"
		#echo "ssh $node ip a |grep ib |grep inet | awk {'print\$2'} | awk '{sub(/.{3}$/,"")}1'"
		ip=`ssh $node ip a |grep ib |grep inet | awk {'print$2'} | awk '{sub(/.{3}$/,"")}1'` # get target ib ip
		echo $ip
		ping -c 1 $ip
		ret=$?
		echo -e " [$node]: IB $ip working: $ret\n"
		...
		let ret=${ret}+${?}
		echo "ret: $ret (copy this part to msg_34(mir/echo))"
