...
//#if DEBUG_SERIAL
#ifdef CONFIG_POPCORN_HYPE
    POP_PRINTF("\tt================= problem ======================\n");
    POP_PRINTF("\tt================================================\n");
    POP_PRINTF("\tt[%d/%d] %s %s(): this thread \"term-poll\"\n",
...
static int term_init(struct kvm *kvm)
{
	...
	for (i = 0; i < TERM_MAX_DEVS; i++) {
		...
		POP_PRINTF("\t\ttry set up dev %d\n", i);
#endif
		if (term_fds[i][TERM_FD_IN] == 0) {
#ifdef CONFIG_POPCORN_HYPE
			POP_PRINTF("\t\t\tset up dev %d\n", i);
#endif
