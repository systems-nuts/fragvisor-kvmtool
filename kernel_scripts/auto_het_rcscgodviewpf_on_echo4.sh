...
#
# auto_het.sh
# Copyright (C) 2018 jackchuang <horenc@vt.edu>
#
# Distributed under terms of the MIT license.
#
repeat() { for i in `seq $2`; do $1; done }
repeat_echo() { for i in `seq $2`; do echo $1; done }
...
yrat=$6
runs=$7
repeat_echo "\$5: $5 \$6: $6 {$xrat},{$yrat} \$7: $7 runs" 5
# just in case export here redaundantly
#servers "export POPCORN_HET_WORKSHARE="{$xrat},{$yrat}""
echo "($HOST_NODE first) export POPCORN_HET_WORKSHARE=\"{$xrat},{$yrat} $runs runs\""

#TSTAMP=`date |sed 's/ /_/g'`
TSTAMP=`date +%Y%m%d_%T | sed 's/:/_/g'`
#FOLDER="share/test_${TSTAMP}_mwpfalllog_inside_rm_kmpcendreduce_${ScriptName}_${xrat}_${yrat}"
#FOLDER="share/test_${TSTAMP}_mwpfalllog_inside_rm_kmpallreduce_smart9_${ScriptName}_${xrat}_${yrat}"
#FOLDER="share/test_${TSTAMP}_mwpfnolog_after_remove_fence_in_endtso_${ScriptName}_${xrat}_${yrat}"
#FOLDER="share/test_${TSTAMP}_mwpfalllog_post_icdcs19_local_cpu_ratio_${ScriptName}_${xrat}_${yrat}"
#FOLDER="share/test_${TSTAMP}_mwpfalllog_post_icdcs19_local_batch_24_144_${ScriptName}_${xrat}_${yrat}"
#FOLDER="share/test_${TSTAMP}_master_post_icdcs19_16_96_${ScriptName}_${xrat}_${yrat}"
#FOLDER="share/test_${TSTAMP}_master_post_icdcs19_18_112_${ScriptName}_${xrat}_${yrat}"
FOLDER="share/test_${TSTAMP}_${HOST_NODE}_perfdata_icdcs19_mwpf_NOLOCKmw_NOLOCKpf_REALbatchoriented_16_96_${ScriptName}_${xrat}_${yrat}"
...
remote_run()
{
	...
	local cur_iter=$6
	# just in case export here redaundantly
	echo "\$5 $5 $xrat \$6 $6 $yrat iter $iter"
	export POPCORN_HET_WORKSHARE="{$xrat},{$yrat}"
	echo "(outer) export POPCORN_HET_WORKSHARE=\"{$xrat},{$yrat} iter ${iter}\""
	...
	#
	# TODO servers+log
	echo "collecting msg statis... ($HOST_NODE)"
	uname -a >> ${2}_popcorn_stat$4
	uname -a >> ${2}_popcorn_stat$4
	uname -a >> ${2}_popcorn_stat$4
	uname -a >> ${2}_dmesg$4
	cat /proc/popcorn_stat >> ${2}_popcorn_stat$4
	cat /proc/popcorn_stat # godview
	#if  [[ $cur_iter > 1 ]]; then
		cat /proc/popcorn_stat # !godview
	#fi
	# so that onlt the first run record
	sudo bash -c dmesg -c >> ${2}_dmesg$4

	echo "collecting msg statis... (fox5)"
	ssh fox5 "uname -a" >> ${2}_popcorn_stat$4
	ssh fox5 "uname -a" >> ${2}_popcorn_stat$4
	ssh fox5 "uname -a" >> ${2}_popcorn_stat$4
	ssh fox5 "uname -a" >> ${2}_dmesg$4
	ssh fox5 "cat /proc/popcorn_stat" >> ${2}_popcorn_stat$4
	ssh fox5 "cat /proc/popcorn_stat" # godview
	#if  [[ $cur_iter > 1 ]]; then
		ssh fox5 "cat /proc/popcorn_stat" # !godview
	#fi
	# so that onlt the first run record
	ssh fox5 "sudo bash -c \"dmesg -c\"" >> ${2}_dmesg$4

	mv ${2}_popcorn_stat$4 $FOLDER # 
	mv ${2}_dmesg$4 $FOLDER #

	if [ -d "$FOLDER" ]; then
		touch $FOLDER/ratio_${xrat}_${yrat}
	else
...
#    #servers "sudo bash -c \"echo 1 > /sys/kernel/debug/tracing/events/popcorn/pgfault_stat/enable\"" # only for mm time
	...
	cat /proc/popcorn_stat # god view
	...
	ssh fox5 "cat /proc/popcorn_stat" #god view
	...
	#
	#
	echo "collecting ${2}_trace... ($HOST_NODE)"
...
for j in `seq $runs`; do
	clear_log
	repeat_echo "$ScriptName iter ${j}/${runs}" 3
	sleep 5
	remote_run "./evaluate-het-rcscgodviewpf_on_echo4.sh -b $3 $xrat $yrat" "$4" $copy_apps_for_statis ${j} $xrat $yrat $j
done
