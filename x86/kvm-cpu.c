...
struct kvm_cpu *kvm_cpu__arch_init(struct kvm *kvm, unsigned long cpu_id)
{
	...
	vcpu = kvm_cpu__new(kvm); /* TODO: this is a malloc */
	if (!vcpu)
		return NULL;
	...
	POP_DPRINTF(pop_get_nid(), "\t\t[%d/%d] %s(): mmap_size %d (3pgs)\n",
					pop_get_nid(), popcorn_gettid(), __func__, mmap_size);

	/* vcpu->vcpu_fd aka (kernel file)anon_inode:kvm-vcpu:0 */
	POP_DPRINTF(pop_get_nid(),
			"\t\t[%d/%d] %s(): mmap(vcpu_fd) anon_inode:kvm-vcpu:%d\n",
						pop_get_nid(), popcorn_gettid(), __func__, cpu_id);

	/***********************************************/
	/* pophype kernel - mm/mmap.c SYS_mmap_pgoff
	 * remote nodes hope to get the same uaddr from origin */
	/***********************************************/
	/* pophype - origin fails to mmap if !MAP_SHARED (because of our DSM?) */
	vcpu->kvm_run = mmap(NULL, mmap_size, PROT_RW, MAP_SHARED, vcpu->vcpu_fd, 0);
	if (vcpu->kvm_run == MAP_FAILED)
		die("unable to mmap vcpu fd");

	POP_DPRINTF(pop_get_nid(), "\t  [%d/%d] %s(): vcpu_id <%d> vcpu %p "
							"mmap got [[[vcpu->kvm_run %p ]]]\n",
							//"mmap got [[[[[vcpu->kvm_run %p %p %p]]]]]\n",
					pop_get_nid(), popcorn_gettid(), __func__, cpu_id, vcpu,
																vcpu->kvm_run);
				//vcpu->kvm_run, vcpu->kvm_run + 0x1000, vcpu->kvm_run + 0x2000); /* not in char 8B */
	...
	POP_DPRINTF(pop_get_nid(), "\t\t[%d/%d] %s(): coalesced_offset %d\n",
					pop_get_nid(), popcorn_gettid(), __func__, coalesced_offset);

/////////dbg working touching vcp on the node
	// don't touch origin's pages otherwise cow has problem
	// all no - hits VM_SHARE
	//if (pop_get_nid() && cpu_id) { // works
	//if (!pop_get_nid() && cpu_id) { // don't touch origin's pages otherwise cow has problem
	//if (!pop_get_nid()) { // 0 touches all
	//if (pop_get_nid()) { // 1 touches all
#define ENFORCE_TOUCH_CPU_RUN 0 // 200602 removed - testing
#if ENFORCE_TOUCH_CPU_RUN
	{
//		volatile char *a = malloc(mmap_size);
//		memcpy((void*)a, vcpu->kvm_run, mmap_size);
//		free((void*)a);
//		POP_DPRINTF(pop_get_nid(), "\t\t<%d> %s(): enforce to touch kvm_vcpu_fault\n",
//															pop_get_nid(), __func__);
		/* what i really wanna write is vmx->exit_reason but it's inside kernel */
		//if (pop_get_nid() && !cpu_id) // test DSM remote writes origin
		//	vcpu->kvm_run->exit_reason = 123;
//		if (!pop_get_nid() && cpu_id) // test DSM origin writes remote
//			vcpu->kvm_run->exit_reason = 321;

		// only touch its pages
		/* if remote __do_fault -> kvm_vcpu_fault cannot work (unlinked),
			this reason (333) will stop lkvm moving forward */
//		if (pop_get_nid() && cpu_id) // test DSM remote writes origin
//			vcpu->kvm_run->exit_reason = 333;
		if (!pop_get_nid() && !cpu_id) // test DSM origin writes remote
			vcpu->kvm_run->exit_reason = 7878;
	}
#endif
	//}
//////////////////////////////////
...
static void kvm_cpu__setup_msrs(struct kvm_cpu *vcpu)
{
	...
	POP_PRINTF("<%lu>=<fd %d> %s(): this cause hanging on mir machines (begin)\n",
										vcpu->cpu_id, vcpu->vcpu_fd, __func__);
#endif

	vcpu->msrs = kvm_msrs__new(100);

#ifdef CONFIG_POPCORN_HYPE
	POP_PRINTF("<%lu>=<fd %d> %s(): this cause hanging on mir machines (end)\n",
										vcpu->cpu_id, vcpu->vcpu_fd, __func__);
#endif
	...
	POP_PRINTF("<%lu>=<fd %d> %s(): KVM_SET_MSRS begin\n",
					vcpu->cpu_id, vcpu->vcpu_fd, __func__);

	if (ioctl(vcpu->vcpu_fd, KVM_SET_MSRS, vcpu->msrs) < 0)
		die_perror("KVM_SET_MSRS failed");

	POP_PRINTF("<%lu>=<fd %d> %s(): KVM_SET_MSRS end\n",
				vcpu->cpu_id, vcpu->vcpu_fd, __func__);
}
...
static void kvm_cpu__setup_fpu(struct kvm_cpu *vcpu)
{
	...
	POP_PRINTF("[%d/%d/%d] %s(): <%lu>=<fd %d> KVM_SET_FPU begin\n",
	...
	POP_PRINTF("[%d/%d/%d] %s(): <%lu>=<fd %d> KVM_SET_FPU end\n",
				pop_get_nid(), getpid(), popcorn_gettid(), __func__,
				vcpu->cpu_id, vcpu->vcpu_fd);
}
...
static void kvm_cpu__setup_regs(struct kvm_cpu *vcpu)
{
	...
	POP_PRINTF("<%lu>=<fd %d> : KVM_SET_REGS begin\n", vcpu->cpu_id, vcpu->vcpu_fd);
	if (ioctl(vcpu->vcpu_fd, KVM_SET_REGS, &vcpu->regs) < 0)
		die_perror("KVM_SET_REGS failed");
	POP_PRINTF("<%lu>=<fd %d> : KVM_SET_REGS end\n", vcpu->cpu_id, vcpu->vcpu_fd);
}

static void kvm_cpu__setup_sregs(struct kvm_cpu *vcpu)
{
	POP_PRINTF("[%d/%d/%d] %s(): <%lu>=<fd %d>: KVM_GET_SREGS begin\n",
			pop_get_nid(), getpid(), popcorn_gettid(), __func__,
			vcpu->cpu_id, vcpu->vcpu_fd);
	if (ioctl(vcpu->vcpu_fd, KVM_GET_SREGS, &vcpu->sregs) < 0)
		die_perror("KVM_GET_SREGS failed");
	POP_PRINTF("[%d/%d/%d] %s(): <%lu>=<fd %d>: KVM_GET_SREGS end\n",
			pop_get_nid(), getpid(), popcorn_gettid(), __func__,
			vcpu->cpu_id, vcpu->vcpu_fd);
	...
	POP_PRINTF("[%d/%d/%d] %s(): <%lu>=<fd %d>: KVM_SET_SREGS begin\n",
			pop_get_nid(), getpid(), popcorn_gettid(), __func__,
			vcpu->cpu_id, vcpu->vcpu_fd);
	if (ioctl(vcpu->vcpu_fd, KVM_SET_SREGS, &vcpu->sregs) < 0)
		die_perror("KVM_SET_SREGS failed");
	POP_PRINTF("[%d/%d/%d] %s(): <%lu>=<fd %d>: KVM_SET_SREGS end\n",
			pop_get_nid(), getpid(), popcorn_gettid(), __func__,
			vcpu->cpu_id, vcpu->vcpu_fd);
}
...
{
	POP_PRINTF("[%d/%d/%d] %s(): <%lu>=<fd %d>:\n",
			pop_get_nid(), getpid(), popcorn_gettid(), __func__,
			vcpu->cpu_id, vcpu->vcpu_fd);
	kvm_cpu__setup_cpuid(vcpu); /* x86/cpuid.c */
