...
#else
	if (!pop_get_nid())
		fprintf(stderr, "(POPHYPE ERROR) %s%s\n", prefix, msg);
	POP_DPRINTF(pop_get_nid(), "\t\t<%d> %s%s\n", pop_get_nid(), prefix, msg);
#endif
}
...
void die_perror(const char *s)
{
    ...
    printf("\t\t<%d> DIE %s\n", pop_get_nid(), s);
    fprintf(stderr, "\t\t<%d> DIE %s\n", pop_get_nid(), s); // NOT tested yet
    //POP_DPRINTF(pop_get_nid(), "<%d> DIE %s\n", pop_get_nid(), s);
#else
	perror(s);
#endif
	exit(1);
...
#ifndef CONFIG_POPCORN_HYPE
		if (!pop_get_nid()) { printf("\t<*> mmap(NULL)\n"); }
#endif
